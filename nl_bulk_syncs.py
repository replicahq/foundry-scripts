...
def generate_table_names():
    ...
    for region in REGIONS:
        for season in SEASONS:
            for day in DAYS:
                tables.append(
                    "model-159019.nl_breakdown.{}_{}_{}".format(region, season, day)
                )
    return tables
...
def create_dataset(table):
    ...
    if response.status_code in [200, 204]:
        ...
        f = open("nl/nl_dataset_rid.txt", "a")
        f.write(f"\n{response.json()['rid']},")
        f.close()
    else:
...
def create_sync(dataset_rid, project, folder, table):
    ...
    if response.status_code in [200, 204]:
        print("SUCCESS: Created Sync: {}".format(response.json()))
        f = open("nl/nl_sync_rid.txt", "a")
        f.write(f"\n{response.json()},")
        f.close()
    else:
